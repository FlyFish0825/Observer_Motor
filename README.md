...
为 `Kp=0.0005、Ki=0.005`，输出Iq参考限制为±10 A。
这组参数用于首次保守测试，实际上板仍需结合电流和阶跃响应继续微调。

电流采样使用5 mΩ分流电阻、24倍模拟增益和1.65 V中点偏置。3.3 V ADC对应理论双向量程约±13.75 A；10 A峰值时ADC约为0.45~2.85 V，因此仍保留约0.45 V端点裕量。这里的10 A仅是控制软件允许的Iq参考上限，不替代硬件过流保护。
...
正常线性调制区，同时防止PI在后级电压缩放时继续积分。速度环Iq限流现为±10 A。
